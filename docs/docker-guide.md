...
```dockerfile
# filepath: loadbalancer/Dockerfile
...
```nginx
# filepath: loadbalancer/nginx.conf
...
```yaml
# filepath: docker-compose.hub.yml
...
services:
  ...
  api:
    image: mean-docker/mean-api:latest
    container_name: mean-api
    restart: always
    networks:
      - mean-network
  ...
  angular:
    image: mean-docker/mean-ui:latest
    container_name: mean-ui
    restart: always
    networks:
      - mean-network
  ...
  nginx:
    image: mean-docker/mean-nginx:latest
    container_name: mean-nginx
    restart: always
    ports:
      - "80:80"
